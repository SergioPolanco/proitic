$(document).ready(function() {
    ...
    $( "[href='/administrator/cooperation/activities/modify/']" ).parent().addClass('active').parent().addClass('open');
    $( "[href='/administrator/cooperation/activities/modify/']" ).parent().parent().parent().addClass('active open hsub');
    $( "[href='/administrator/cooperation/activities/modify/']" ).parent().parent().parent().parent().parent().addClass('active open hsub');
    ...
    $( document ).on( 'click', 'i.fa-pencil.bigger-130', function(e) {
        $.ajax({
            url : '/administrator/cooperation/activities/search/',
            type : 'post',
            ...
            success: function(data) {
                if( data.status === 'False' ) {
                  ...
                  $( 'input[name="activityname"]' ).val( data.name );
    ...
    $(document).on( 'submit' , "form[name='form-new-project']" ,function () {
        ...
        if( validateblank() ) {
            ...
            $.ajax({
                url : '/administrator/cooperation/activities/update/',
                type : 'post',
                data : formData,
                async : true,
                contentType: false,
                ...
                success: function(data) {
                  console.log( data.active.toString() );
                    if( data.status === 'False' ) {
                      ...
                      $( '#sample-table-2 tbody tr[name=' + data.id + '] td:nth-child(3)' ).text( jQuery.trim(data.name).substring(0, 88).split(" ").slice(0, -1).join(" ") + "..." );
...
function validateblank() {
    var validate = false;
    $(" #trumbowyg-demo, input[name='activityname'] ").each(function(){
        if( $( this ).val() === '' ) {
            validate = true;
            $( this ).parent().parent().addClass('has-error');
        }
