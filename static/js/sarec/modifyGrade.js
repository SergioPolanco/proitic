...
function load(){
  $( "[href='/administrator/graduationWork/modifyGrade/']" ).parent().addClass('active').parent().addClass('open');
  $( "[href='/administrator/graduationWork/modifyGrade/']" ).parent().parent().parent().addClass('active open hsub');
  $( "[href='/administrator/graduationWork/modifyGrade/']" ).parent().parent().parent().parent().parent().addClass('active open hsub');
  ...
  $("#file_grade_modify").on("change", function(e){
    var filename = e.target.files[0].name;
    $("#result_file_grade_modify").html(filename);
  });
  ...
  $(".ace-icon.fa.fa-pencil.bigger-130").on('click', function(){
    var idGrade= $.trim($($(this).parent().parent().parent().parent().children(":nth-child(1)")).text());
    $("#idGrade").val(idGrade);
    var formData = new FormData( $( "form[name='form-id-grade']" )[0] );

    $.ajax({
            url : '/administrator/graduationWork/viewGrade/',
            type : 'post',
            data : formData,
            async : true,
            contentType: false,
            ...
            success: function(data) {
              if(data.status == 'True'){
                ...
                $("#result_file_grade_modify").html(data.filename);
  ...
  $("#form-modificar-grado").bootstrapValidator({
    excluded: [':disabled'],
    fields:{
      txtEstudiantes:{
        validators:{
    ...
    e.preventDefault();
    var formData = new FormData( $( "form[name='form-modificar-grado']" )[0] );
    formData.append("txtTutor", $(".chosen-select").val());
    formData.append('descripcion', $('#trumbowyg-demo').val());
      $.ajax({
          url : '/administrator/graduationWork/updateGrade/',
          type : 'post',
          data : formData,
          async : true,
          contentType: false,
