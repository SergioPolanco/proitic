...
function load(){
  $( "[href='/administrator/graduationWork/addPosGrade/']" ).parent().addClass('active').parent().addClass('open');
  $( "[href='/administrator/graduationWork/addPosGrade/']" ).parent().parent().parent().addClass('active open hsub');
  $( "[href='/administrator/graduationWork/addPosGrade/']" ).parent().parent().parent().parent().parent().addClass('active open hsub');
  ...
  $("#file_posgrade").on("change", function(e){
    console.log(e.target.files[0].name);
    var filename = e.target.files[0].name;
    $("#result_file_posgrade").html(filename);
  });
  if ($.trumbowyg) {
      var configurations = {
          core: {},
  ...
  $("#form-nuevo-posgrado").bootstrapValidator({
    excluded: [':disabled'],
    fields:{
      txtEstudiantes:{
        validators:{
    ...
    e.preventDefault();
    var formData = new FormData( $( "form[name='form-nuevo-posgrado']" )[0] );
    formData.append("txtTutor", $(".chosen-select").val());
    formData.append('descripcion', $('#trumbowyg-demo').val());
      $.ajax({
          url : '/administrator/graduationWork/insertPosGrade/',
          type : 'post',
          data : formData,
          async : true,
          contentType: false,
